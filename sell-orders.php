...
<head>
    ...
    $(document).ready(function() {
        $('.deliverButton').on('click', function(e) {
            ...
            var id = $(this).data('id'); // Use 'id' as the variable name
            var status = 'delivered';

            $.post('deliver_status.php', { id: id, status: status }, function(response) {
...
<body>
	...
	<div class="container mt-150">
        <div class="row">
            <table class="table table-striped">
                ...
                $sql = "SELECT id, user_id, image, amount, status, payment_id, address, delivery_date FROM orders WHERE shopname='$shop' ORDER BY time DESC";
	...
	if ($result->num_rows > 0) {
		...
		while ($row = $result->fetch_assoc()) {
			...
			echo '<a href="#" class="cart-btn deliverButton" data-toggle="modal" data-target="#orderModal' . $row['id'] . '" data-id="' . $row['id'] . '">Deliver</a>';
